...
(defroutes routes*
  (PUT "/locations/:location-id" {:keys [auth/user body db params]}
    {:status 200
     :body   (v/deref! (locations/save db user (assoc body :id (UUID/fromString (:location-id params)))))})
  (POST "/locations" {:keys [auth/user body db]}
    {:status 201
     :body   (v/deref! (locations/save db user body))})
  (GET "/locations" {:keys [db]}
    {:status 200
     :body   (v/deref! (locations/select-for-admin db))})
  (GET "/shows/:show-id" {:keys [db params]}
    (if-let [show (v/deref! (shows/find-by-id db (UUID/fromString (:show-id params))))]
      {:status 200
       :body   show}
      {:status 404
  ...
  (PUT "/shows/:show-id" {:keys [auth/user body db params]}
    {:status 200
     :body   (v/deref! (shows/save db user (assoc body :id (UUID/fromString (:show-id params)))))})
  (DELETE "/shows/:show-id" {:keys [auth/user db params]}
    (v/deref! (shows/save db user {:id (UUID/fromString (:show-id params)) :deleted? true}))
    {:status 204})
  (POST "/shows" {:keys [auth/user body db]}
    {:status 201
     :body   (v/deref! (shows/save db user body))})
  (GET "/shows" {:keys [db]}
    {:status 200
     :body   (v/deref! (shows/select-for-admin db))}))
